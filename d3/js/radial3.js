...
var rowConverter = function(d) {
	...
	if(d.med_income_00_pctile <= 30) {
		d.income_group = "low";
	} else if(d.med_income_00_pctile <= 70) {
...
d3.csv("data/nola_viz_data.csv", rowConverter, function(tracts) {
	...
	var drawIncomeLabelArc = function(income_group) {
		...
		var labelArc = d3.arc()
			.outerRadius(radius-(25 + thickness*(levels+1)))
			.innerRadius(radius-(25 + thickness*(levels+1)))
	...
	var drawDamageLabelArc = function(damage_level) {
		...
		var outerRadius = (radius - ((levels-damage_level)*(thickness+level_gap)))+level_gap;
		var innerRadius = outerRadius-level_gap;
